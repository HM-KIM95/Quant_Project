...
tickers = ["005930.KS", "005380.KS", "058610.KS", "278470.KS", "034020.KS", "277810.KS", "087010.KS", "AAPL", "MSFT", "GOOGL", "AMZN", "TSLA", "META", "QQQ", "SCHD", "JEPQ"]
...
# S&P500 지수 비교
sp500 = yf.download("^GSPC", start="2015-01-01", auto_adjust=False)["Adj Close"]
sp500_return = sp500.iloc[-1] / sp500.iloc[0] - 1
print("S&P500 수익률: {:.2f}%".format(float(sp500_return) * 100 if not hasattr(sp500_return, "iloc") else float(sp500_return.iloc[0]) * 100))

# NASDAQ 지수 비교
nasdaq = yf.download("^IXIC", start="2015-01-01", auto_adjust=False)["Adj Close"]
nasdaq_return = nasdaq.iloc[-1] / nasdaq.iloc[0] - 1
print("NASDAQ 수익률: {:.2f}%".format(float(nasdaq_return) * 100 if not hasattr(nasdaq_return, "iloc") else float(nasdaq_return.iloc[0]) * 100))
...
plt.plot(nasdaq / nasdaq.iloc[0], label="NASDAQ")
ax = plt.gca()
