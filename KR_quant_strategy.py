...
tickers = ["005930.KS", "005380.KS", "058610.KS", "278470.KS", "034020.KS", "277810.KS", "087010.KS"]

# 2. 가격 데이터 다운로드
data = yf.download(tickers, start="2015-01-01", auto_adjust=False)["Adj Close"]

# 3. 네이버 금융에서 PER, PBR 가져오기
def get_fundamental_from_naver(ticker):
    # 종목코드는 '.KS' 제거
...
pbr = {}
for t in tickers:
    per_val, pbr_val = get_fundamental_from_naver(t)
    per[t] = per_val
    pbr[t] = pbr_val
